...
class Student:
    ...
    # This part is image labels setting start
        ...
        dep_combo["values"]=("Select Department","CSE","CIVIL","MECHANICAL","ELECTRICAL","ETC")
        dep_combo.current(0)
        ...
        cou_combo["values"]=("Select Course", "DIPLOMA")
        cou_combo.current(0)
        ...
        year_combo["values"]=("Select Year","2022-25","2023-26","2024-27","2025-28")
        ...
        "Student ID", "Name", "Dep", "Course", "Year", "Sem", "Div", "Gender", "DOB", "Mob-No", "Address", "Roll-No",
        "Email", "Teacher", "PhotoSample"), xscrollcommand=scroll_x.set, yscrollcommand=scroll_y.set)
    ...
    def add_data(self):
        if self.var_dep.get() == "Select Department" or self.var_course.get == "Select Course" or self.var_year.get() == "Select Year" or self.var_semester.get() == "Select Semester" or self.var_std_id.get() == "" or self.var_std_name.get() == "" or self.var_div.get() == "" or self.var_roll.get() == "" or self.var_gender.get() == "" or self.var_dob.get() == "" or self.var_email.get() == "" or self.var_mob.get() == "" or self.var_address.get() == "" or self.var_teacher.get() == "":
            ...
            try:
                conn = pymysql.connect(user='root', password='root', host='localhost', database='face_recognition',
                                       port=3306)
                mycursor = conn.cursor()
                mycursor.execute(
                    "INSERT INTO student (Student_ID, Name, Department, Course, Year, Semester, Division, Gender, DOB, Mobile_No, Address, Roll_No, Email, Teacher_Name, PhotoSample) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
                    (
                        self.var_std_id.get(),
    ...
    def fetch_data(self):
        ...
        mycursor = conn.cursor()
        mycursor.execute(
            "SELECT Student_ID, Name, Department, Course, Year, Semester, Division, Gender, DOB, Mobile_No, Address, Roll_No, Email, Teacher_Name, PhotoSample FROM student")
        data = mycursor.fetchall()

        if len(data) == 0:
            print("No data found in database.")

        self.student_table.delete(*self.student_table.get_children())  # Clear old records
    ...
    def get_cursor(self, event=None):
        ...
        data = content["values"]
        if data and len(data) >= 15:  # Ensure data has enough elements
            self.var_std_id.set(data[0])
            self.var_std_name.set(data[1])
            self.var_dep.set(data[2])  # Removed comma
            self.var_course.set(data[3])  # Removed comma
            ...
            self.var_radio1.set(data[14] if len(data) > 14 else "")
        ...
        if self.var_std_id.get() == "":
            ...
            try:
                ...
                if delete:
                    ...
                    val = (self.var_std_id.get(),)  # Ensure it's a tuple
                    mycursor.execute(sql, val)
                    conn.commit()
                    conn.close()  # Properly close connection after successful commit

                    messagebox.showinfo("Delete", "Successfully Deleted!", parent=self.root)

                    self.fetch_data()  # Refresh table dynamically
                    self.reset_data()

                else:
                    return  # Cancel delete operation
    ...
    def generate_dataset(self):
        if self.var_dep.get() == "Select Department" or self.var_course.get() == "Select Course" or self.var_year.get() == "Select Year" or self.var_semester.get() == "Select Semester" or self.var_std_id.get() == "" or self.var_std_name.get() == "" or self.var_div.get() == "" or self.var_roll.get() == "" or self.var_gender.get() == "" or self.var_dob.get() == "" or self.var_email.get() == "" or self.var_mob.get() == "" or self.var_address.get() == "" or self.var_teacher.get() == "":
            ...
            try:
                ...
                if existing:
                    student_uid = self.var_std_id.get()
                    my_cursor.execute(
                        "UPDATE student SET Name=%s, Department=%s, Course=%s, Year=%s, Semester=%s, Division=%s, Gender=%s, DOB=%s, Mobile_No=%s, Address=%s, Roll_No=%s, Email=%s, Teacher_Name=%s, PhotoSample=%s WHERE Student_ID=%s",
                        (
                            self.var_std_name.get(), self.var_dep.get(), self.var_course.get(), self.var_year.get(),
                            self.var_semester.get(), self.var_div.get(), self.var_gender.get(), self.var_dob.get(),
                            self.var_mob.get(), self.var_address.get(), self.var_roll.get(), self.var_email.get(),
                            self.var_teacher.get(), self.var_radio1.get(), student_uid
                        ))
                    conn.commit()
                else:
                ...
                face_classifier = cv2.CascadeClassifier("Images_GUI/haarcascade_frontalface_default.xml")
                ...
                while True:
                    ...
                    if cropped is not None:
                        ...
                        file_path = f"data_img/student.{student_uid}.{img_id}.jpg"
