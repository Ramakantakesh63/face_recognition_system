    ...
    def face_recog(self):
                def draw_boundray(img, classifier, scaleFactor, minNeighbors, color, text, clf):
                        ...
                        for (x, y, w, h) in featuers:
                            ...
                            print(f"Predicted ID: {id}, Confidence: {predict}")
                            ...
                            cursor.execute("SELECT Name FROM student WHERE Regd_no = %s", (id,))
                            n = cursor.fetchone()
                            print(n)  # Debugging output

                            if n:
                                n = "+".join(map(str, n))  # Safely convert tuple to string
                            else:
                                n = "Unknown"  # Default value when no match is found

                            cursor.execute("select Roll from student where Regd_no= %s",(id,))
                            r = cursor.fetchone()
                            if r:
                                r = "+".join(map(str, r))
                            else:
                            ...
                            cursor.execute("select Regd_no from student where Regd_no= %s",(id,))
                            i = cursor.fetchone()
                            if i:
                                i = "+".join(map(str, i))
                            else:
                            ...
                            if confidence > 65:
                                cv2.putText(img, f"Regd_no:{i}", (x, y - 80), cv2.FONT_HERSHEY_COMPLEX, 0.8,
                                            (64, 15, 223), 2)
                                cv2.putText(img, f"Name:{n}", (x, y - 55), cv2.FONT_HERSHEY_COMPLEX, 0.8, (64, 15, 223),
                                            2)
                                cv2.putText(img, f"Roll-No:{r}", (x, y - 30), cv2.FONT_HERSHEY_COMPLEX, 0.8,
                                            (64, 15, 223), 2)
