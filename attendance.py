...
class Attendance:
    ...
    def __init__(self, root):
        ...
        # Student id
        studentId_label = Label(left_frame, text="Regd_no:", font=("verdana", 12, "bold"), fg="navyblue", bg="white")
        studentId_label.grid(row=0, column=0, padx=5, pady=5, sticky=W)

        studentId_entry = ttk.Entry(left_frame, textvariable=self.var_regd, width=15, font=("verdana", 12, "bold"))
        ...
        # create table
        self.attendanceReport_left = ttk.Treeview(table_frame,column=("Regd_no", "Roll_No", "Name", "Time", "Date", "Attend"),xscrollcommand=scroll_x.set, yscrollcommand=scroll_y.set)
        ...
        self.attendanceReport_left.heading("Regd_no", text="Regd_no")
        ...
        # Set Width of Colums
        self.attendanceReport_left.column("Regd_no", width=100)
        ...
        # create table
        self.attendanceReport = ttk.Treeview(table_frame, column=("Regd_no", "Roll_No", "Name", "Time", "Date", "Attend"),xscrollcommand=scroll_x.set, yscrollcommand=scroll_y.set)
        ...
        self.attendanceReport.heading("Regd_no", text="Regd_no")
        ...
        # Set Width of Colums
        self.attendanceReport.column("Regd_no", width=100)
        ...
        self.attendanceReport.bind("<ButtonRelease>")
    ...
    def update_data(self):
            if self.var_regd.get() == "" or self.var_roll.get == "" or self.var_name.get() == "" or self.var_time.get() == "" or self.var_date.get() == "" or self.var_attend.get() == "Status":
                ...
                try:
                    Update = messagebox.askyesno("Update", "Do you want to Update this Student Attendance!",
                                                 parent=self.root)
                    if Update > 0:
                        conn = pymysql.connect(username='root', password='root', host='localhost', database='face_recognition', port=3306)
                        mycursor = conn.cursor()
                        mycursor.execute(
                            "update stdattendance set std_regd=%s,std_roll_no=%s,std_name=%s,std_time=%s,std_date=%s,std_attendance=%s where std_id=%s",
                            (
                                self.var_regd.get(),
                                self.var_roll.get(),
                                self.var_name.get(),
    ...
    def delete_data(self):
                if self.var_regd.get() == "":
                    ...
                    try:
                        ...
                        if delete > 0:
                            conn = pymysql.connect(username='root', password='root', host='localhost',
                                                           database='face_recognition', port=3307)
                            mycursor = conn.cursor()
                            sql = "delete from stdattendance where std_id=%s"
                            val = (self.var_regd.get(),)
