...
class Attendance:
    ...
    def __init__(self,root):
        ...
        insert_btn = Button(btn_frame, command=self.InsertAction, text="INSERT", width=12,
                            font=("verdana", 12, "bold"), fg="white", bg="navyblue")
        insert_btn.grid(row=0,column=4,padx=6,pady=8,sticky=W)
        ...
        if self.var_id.get()=="" or self.var_roll.get=="" or self.var_name.get()=="" or self.var_time.get()=="" or self.var_date.get()=="" or self.var_attend.get()=="Status":
            ...
            try:
                ...
                if Update > 0:
                    ...
                    mycursor.execute("update stdattendance set Roll_No=%s,Name=%s,time=%s,date=%s,attendance=%s where Student_ID=%s",(
                    self.var_roll.get(),
                    self.var_name.get(),
                    self.var_time.get(),
                    self.var_date.get(),
        ...
        if self.var_id.get()=="":
            ...
            try:
                ...
                if delete>0:
                    conn = pymysql.connect(user='root', password='root',host='localhost',database='face_recognition',port=3306)
                    mycursor = conn.cursor()
                    sql="delete from stdattendance where Student_ID=%s"
                    val=(self.var_id.get(),)
        ...
        if self.var_id.get() == "" or self.var_roll.get() == "" or self.var_name.get() == "" or self.var_time.get() == "" or self.var_date.get() == "" or self.var_attend.get() == "Status":
            ...
            try:
                ...
                with open("attendance.csv", "a", newline="\n") as f:
                    f.writelines(
                        f"\n{self.var_id.get()}, {self.var_roll.get()}, {self.var_name.get()}, {self.var_time.get()}, {self.var_date.get()}, {self.var_attend.get()}")

                messagebox.showinfo("Success", "All Records Saved in Database & CSV File!", parent=self.root)
    ...
    def action(self):
        if self.var_id.get() == "" or self.var_roll.get() == "" or self.var_name.get() == "" or self.var_time.get() == "" or self.var_date.get() == "" or self.var_attend.get() == "Status":
            ...
            try:
                ...
                mycursor = conn.cursor()
                mycursor.execute(
                    "update stdattendance set Roll_No=%s,Name=%s,time=%s,date=%s,attendance=%s where Student_ID=%s", (
                        self.var_roll.get(),
                        self.var_name.get(),
                        self.var_time.get(),
                        self.var_date.get(),
